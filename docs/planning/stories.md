...
- **NEW-12** As a manager, I want to compose my weekly summary and push it up the hierarchy (ASM to BM, DM to NSM), with my superior receiving it in-app, so that upward reporting is one click, not an evening. (Two-sided loop: AC covers send + receive.) · `manager` · needs: dashboards data, hierarchy
- **NEW-23** As a manager, I want my weekly summary auto-drafted from my team's actual data for me to edit before pushing, so that the report writes itself and I only add judgment. · `manager` `ai` · needs: NEW-12, AI service
...
- **NEW-14** As a manager or PM, I want my own field activity logged (check-ins, joint calls), visible up my hierarchy, so that supervision is provable the same way rep work is. (May split capture/visibility at refinement if fat.) · `manager` · needs: FID-12 machinery reused
