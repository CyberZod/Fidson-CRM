...
- **FID-107** As a manager (PM/NSM/exec), I want to broadcast directives to a chosen audience and see who acknowledged, so that "I never saw it" stops being possible. · `manager` · needs: hierarchy; pairs with NEW-3
...
- **NEW-10** As a manager, I want to send a direct message to one rep in-app, so that field guidance lives where field work lives, not in WhatsApp. · `manager` · needs: hierarchy
- **NEW-11** As a manager (ASM), I want a coaching log per rep (notes, 1:1s, micro-coaching nudges), so that development is deliberate and survives manager changes. · `manager` · needs: hierarchy, FID-15 data
- **NEW-12** As a manager, I want to compile and push my weekly summary up the hierarchy (ASM to BM, DM to NSM), so that upward reporting is one click, not an evening. · `manager` · needs: dashboards data, hierarchy
- **NEW-13** ⚠ As a senior manager, I want to see how my managers supervise (reviews done, approvals turnaround, coaching activity), so that management performance is measured, not assumed. · `manager` · needs: NEW-14 (manager activity data); sensitivity question
- **NEW-14** As a manager or PM, I want my own field activity logged (check-ins, joint calls), visible up my hierarchy, so that supervision is provable the same way rep work is. · `manager` · needs: FID-12 machinery reused
- **NEW-15** As an FSM, I want distributor management (stock health, days-to-stockout, reorders, quarterly audits, onboarding), so that channel supply problems surface before they cost sales. · `manager` `orders` · needs: distributor data (seed), FID-33/34 order data over time
