...
- **FID-37** As a rep, I want to see my customer's statement of account, so that I walk into money conversations informed. (Ships as a deep link into the existing SOA app; the native in-app view is NEW-32.) · `rep-app` `orders` · needs: SOA deep link
- **NEW-32** As a rep, I want the statement of account shown natively inside the app, so that money conversations don't require jumping to a second app. (Follow-up to FID-37.) · `rep-app` `orders` · needs: FID-37, FID-34 ERP sync
...
- **NEW-1** As a rep, I want a My Customers view with a 360 per customer (visit history, commitments, key people), so that I never open cold with someone we've known for years. (This is the prototype's CustomersView, productized. Ships on visit data alone; the money side is NEW-33. AI suggestions on the same data are FID-41, separate.) · `rep-app` · needs: FID-14
- **NEW-33** As a rep, I want my customer 360 to also show orders, payment status and lifetime value, so that the profile covers the money side of the relationship, not just visits. (Follow-up to NEW-1.) · `rep-app` `orders` · needs: NEW-1, FID-33
...
- **NEW-13** ⚠ As a senior manager, I want to see, for each manager under me, how well they run their team: how quickly they decide approvals sitting on their desk, whether they review their reps' daily reports, how often they coach, and how often they go to the field themselves, so that how someone manages is measured from the record, not assumed from impressions. (Each measure appears via AC once its source story ships: approvals from the approval flow, coaching from NEW-11, report reviews from FID-101, field presence from NEW-14.) · `manager` · needs: those source stories; sensitivity question
- **NEW-14** As a manager or PM, when I go to the field myself (a hospital call, a distributor visit, a market check), I want to check in and log that visit the same way a rep does, with my log visible to my own superior, so that field presence is provable at every level of the company, not just for reps. (Fidson's URS: "PMs should also be able to check in"; accompaniments/joint calls stay in FID-104.) · `manager` · needs: FID-12 machinery reused
...
- **NEW-24** As an FSM, I want each distributor's stock health visible (levels, days-to-stockout), so that supply problems surface before shelves go empty. (Ships on manually entered stock figures; automatic feeds are NEW-34.) · `manager` `orders` · needs: NEW-15
...
- **NEW-34** As an FSM, I want stock health computed automatically from distributors' order flow instead of manual entries, so that the numbers stay current without anyone typing them. (Follow-up to NEW-24.) · `manager` `orders` · needs: NEW-24, FID-33
...
- **FID-43** As a DM or NSM, I want churn-risk and forecasts with explained drivers, so that I act on next quarter before it happens. (Ships on Fidson-provided sample data, per JBS §4.1; the live-history version is NEW-35.) · `ai` `manager` · needs: AI service, sample dataset
- **NEW-35** As a DM or NSM, I want forecasts and churn scores driven by our real accumulated field and sales history, so that the numbers are trustworthy enough to bet a quarter on. (Follow-up to FID-43.) · `ai` `manager` · needs: FID-43, months of live visit/order data
