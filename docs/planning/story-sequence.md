...
Supersedes the v1 "waves" arrangement. This version orders stories ONLY by hard dependency: story B appears under story A only if B cannot be built or make sense until A is done. Seeds (data imports) and machinery (approval engine, notification engine, AI service) are deliberately EXCLUDED: refinement resolves those, per the absorb-or-extract rule, and they do not order stories. Where a capability ships shallow first and deepens later, the deepening is its own follow-up story with normal dependencies (NEW-32 native SOA, NEW-33 360 orders panel, NEW-34 auto stock health, NEW-35 forecasts on live data); no story is ever half-done or reopened.
...
- FID-11 sign in with Entra (root)
  - FID-14 core visit record
    ...
    - NEW-1 customer 360 (visits-only)
      - NEW-33 360 orders panel (also needs FID-33)
    - FID-41 rep next-best-action (reads visit data)
    - FID-95 standard reports → FID-96 custom builder → FID-97 scheduled distribution
    - FID-33 place order
      - FID-35 discount approvals · FID-36 invoices · NEW-19 sales admin routing · FID-108 campaign ROI (needs order data)
...
- NEW-15 distributor directory → NEW-24 stock health (manual figures) → NEW-25 push reorder; NEW-24 → NEW-34 auto stock health (also needs FID-33); NEW-15 → NEW-26 quarterly audit
- FID-43 churn/forecast (sample data per JBS §4.1) → NEW-35 forecasts on live history (needs months of FID-14/33 data)
- FID-37 SOA deep link → NEW-32 native SOA view (also needs FID-34 ERP sync)
...
- Standalone roots, no parents and no children blocking on them at story level: FID-23 offline content, FID-29 expenses, NEW-2 performance vs targets, NEW-7 quotes ⚠, NEW-8 complaints ⚠, NEW-9 focus products, NEW-10 1:1 messages, NEW-11 coaching log, FID-106 promo requests, NEW-16 board pack ⚠
...
- ~~FID-30~~ notification center: composite; decomposes into per-role notification stories (see stories.md), each written once that role's notification types are facts, each depending on its own feeders. Shared screen machinery = enabler. Same treatment as FID-41's per-role split.
...
Rule of thumb: each replacement can ship a foundation version (view + basic edit) early, per the JBS week-6 commitment, with the full builder as a later follow-up story.

## What this graph is for

Milestones pick stories in any order that respects the arrows: a story may ship in any milestone at-or-after all its parents. The JBS 10-week schedule is one legal such picking (verified against this graph; its early pulls: conflict queue, complaints, 360, admin foundation, are all shallow-first versions or roots, not violations). FID-27 delegation and FID-29 expenses etc. sit wherever their milestone wants them.
